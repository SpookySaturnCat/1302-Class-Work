...
public class Game {
	...
	/*
	 * I either want to expand the below method to force the user to use the cash
	 * register to input the correct order and give proper change right after
	 * getting the order, I don't see why these methods would need to be broken up
	 */
	// Creates 'randomized' customer drink orders
	// NEED TO CLEAN THIS JAWN UP EW ITS UGLY
	public Drink customerOrder(int limit) {
		// Initialize variable
		int randomNumber = 0, choice = 0;
		String path = "src/coffeeshop/CustomerDialog.txt";
		...
		String drinkName =  drinks.get(randomNumber);
		...
		if (randomNumber == 1 || randomNumber == 2) {
			// String array with the first row being starting values and [1][index] being
			// the ending value
			String[][] options = {
		...
		try {
		System.out.print("?\n\nEnter the correct drink number: \n\n");
		
		// For loop to print possible drinks 
		for (int i = 1; i <= drinks.size(); i++) {
			System.out.printf("%d: %s%n", i, drinks.get(i - 1));
		}
